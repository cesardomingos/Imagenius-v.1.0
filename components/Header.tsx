...
const Header: React.FC<HeaderProps> = ({ onReset, hasImages, goToGallery, credits, onOpenStore, currentUser, onOpenAuth, onLogout, onOpenProfile, hasNewAchievement = false, onOpenAbout, onOpenFAQ, onOpenTutorial }) => {
  ...
  return (
    <>
      <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800 sticky top-0 z-40" role="banner">
        ...
        <nav className="flex items-center gap-4 md:gap-6 lg:gap-8" role="navigation" aria-label="Navegação principal">
          {/* Quote/Tagline */}
          ...
          <div className="hidden md:flex items-center gap-4" role="list">
            {/* Sobre Button */}
            {onOpenAbout && (
              <Tooltip content="Saiba mais sobre o Imagenius e nossa missão">
                <button
                  onClick={onOpenAbout}
                  ...
                  aria-label="Abrir página sobre o Imagenius"
                  role="listitem"
                >
                  Sobre
                </button>
            ...
            {/* FAQ Button */}
            {onOpenFAQ && (
              <Tooltip content="Perguntas frequentes e respostas">
                <button
                  onClick={onOpenFAQ}
                  className="text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 font-bold text-xs uppercase tracking-widest transition-colors"
          ...
          <Tooltip content="Seus créditos disponíveis. 1 crédito = 1 imagem gerada. Clique para comprar mais créditos.">
            <div 
              ...
              role="button"
              tabIndex={0}
              aria-label={`Você tem ${credits} créditos. Clique para comprar mais.`}
              aria-live="polite"
              data-tour="credits"
            >
          ...
          <div className="hidden md:flex items-center gap-3">
            {currentUser ? (
              <>
                <Tooltip content="Acessar seu perfil, conquistas e configurações">
                  <button
                    ...
                    aria-label={`Abrir perfil do usuário ${currentUser.email}`}
                    aria-haspopup="dialog"
                  >
                  {hasNewAchievement && (
                    <span 
                      className="absolute -top-1.5 -right-1.5 w-3 h-3 bg-red-500 rounded-full ring-2 ring-white dark:ring-slate-900" 
                      aria-label="Você tem uma nova conquista"
                      role="status"
                    />
                  )}
                ...
                <Tooltip content="Fazer logout da sua conta">
                  <button 
                    onClick={onLogout}
                    ...
                    aria-label="Fazer logout da sua conta"
                  >
                    Sair
                  </button>
                </Tooltip>
              ...
              <Tooltip content="Fazer login ou criar uma conta">
                <button 
                  onClick={onOpenAuth}
                  ...
                  aria-label="Fazer login ou criar uma conta"
                  aria-haspopup="dialog"
                >
                  Entrar
                </button>
    ...
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-t border-slate-200 dark:border-slate-700 z-[100] safe-area-inset-bottom" role="navigation" aria-label="Navegação mobile">
      <div className="flex items-center justify-around h-16 px-1" role="list">
        ...
        {/* Perfil */}
        <Tooltip content={currentUser ? "Acessar seu perfil, conquistas e configurações" : "Fazer login ou criar uma conta"} position="top">
          <button
            onClick={() => {
              if (currentUser) {
                onOpenProfile();
          ...
          {hasNewAchievement && (
            <span 
              className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-red-500 rounded-full ring-2 ring-white dark:ring-slate-900" 
              aria-label="Você tem uma nova conquista"
              role="status"
            />
          )}
