...
const App: React.FC = () => {
  ...
  const handleGenerateBatch = async (selectedPrompts: string[]) => {
    ...
    // Transition to gallery immediately to show progress
    setStep('gallery');
              ...
              <div className="space-y-10 animate-in zoom-in-95 duration-500">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                  <div>
                    ...
                    {batchStatus && (
                        <p className="text-sm font-bold text-indigo-600 mt-2 animate-pulse">
                            Gênio em ação: Gerando obra {batchStatus.current} de {batchStatus.total}...
                        </p>
                    )}
                  </div>
